#!/bin/bash
# Adapted from examples/raspberry_pi/pico2/build_firmware_pico.sh
...
PICO2_DIR="${ROOT_DIR}/examples/riscv/pico2"
...
echo "Cross compiling ExecuTorch baremetal RISCV..."

cmake -B "${EXECUTORCH_BUILD_DIR}" \
  -DCMAKE_TOOLCHAIN_FILE="${ROOT_DIR}/examples/riscv/riscv32-unknown-elf-gcc.cmake" \
  -DTARGET_CPU=rv32imc \
  -DEXECUTORCH_BUILD_RISCV_BAREMETAL=ON \
